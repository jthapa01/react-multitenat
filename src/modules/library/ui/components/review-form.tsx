...
export const ReviewForm = ({ productId, initialData }: Props) => {
    ...
    const updateReview = useMutation(trpc.reviews.update.mutationOptions({
        onSuccess: () => {
            // "forget" cached data and refetch it from the server.
            queryClient.invalidateQueries(trpc.reviews.getOne.queryOptions({
                productId,
            }))
            setIsPreview(true);
        ...
        <Form {...form}>
            ...
            >
                <p className="font-medium">
                    {isPreview ? "Your review has been submitted." : "Please leave a review for this product."}
                </p>
                <FormField
...
// Client Component → tRPC Client → HTTP Request → Server tRPC Router → Database
//               ←             ←              ←                    ←